...
<?php
  ...
  if(isset($_POST['submit'])){
    if(!is_logged_in()){
    	echo "Please login to your account before generating a payment invoice. <a href='login.php'>Login</a>";
    	exit;
    }   
    else{
    	process_request();
  ...
  function process_request(){
  	$amt= stripslashes(strip_tags($_POST['month']));
  	try{
           $userid = $_SESSION['userid'];
           $db = $GLOBALS['db'];
           $stmt = $db->prepare('SELECT * FROM users WHERE userid = :userid');
           ...
           $mntbill = $row['mntbill'];
           $ppr = gen_ppr();
           $tx_date = date("M j , Y g:i A");
           $bank_charges = BANKCHARGES;
           $total_amt = $mntbill + $bank_charges;


  		      $db = $GLOBALS['db'];
		        $stmt = $db->prepare('INSERT INTO postpaid_invoice(cust_name,cust_id,h_addr,ppr,month,bill_amt,bank_charges,total_amt,tx_date,tx_status) VALUES (:cust_name,:cust_id,:h_addr,:ppr,:month,:bill_amt,:bank_charges,:total_amt,:tx_date,:tx_status)');
		        $stmt->execute(array(
		 				 ':cust_name' => $cust_name,
						 ':cust_id' => $userid,
						 ':h_addr' => $h_addr, 			
	  ...
	  redirect("postpaid_invoice.php?ppr=$ppr");
 }
	...
	<div id="content">
		<div id='login_div3'>
		  ...
		  <p class="my-color">Use the invoice generated after filling your payment details here to pay 
			...
			payment, the bank will issue you a receipt validating your payment.</p>
		  
	<form  role="form" id="prepaid-form" method="post" action="paybill.php"style="width:400px;"> 
		...
		<?php 
			...
			<tr>
					<td><span id="sprytextfield2">
						<label for="name">Bill amount<br/>
							<input type="text" class="StyleTxtField2" id="amt" value="<?php echo $GLOBALS['mntbill'];?>" disabled>
						</label></span>
					</td>
			</tr>
			<tr>
			...
			<tr>
					<td><span id="sprytextfield2">
						<label for="cust_name">Customer Name<br/>
							<input type="text" class="StyleTxtField2" id="cust_name" value="<?php echo $GLOBALS['cust_name'];?>" disabled>
						</label></span>
					</td>
			</tr>
			<tr>
			...
			<tr>
					<td><span id="sprytextfield2">
						<label for="cust_id">Customer ID<br/>
							<input type="text" class="StyleTxtField2" id="cust_id" value="<?php echo $GLOBALS['userid'];?>" disabled>
						</label></span>
					</td>
			</tr>
			<tr>
