...
<?php
   ...
   if(isset($_POST['submit'])){
     ...
     if(validate_input()){
	     ...
	     try{
	      $stmt = $db->prepare("INSERT INTO postpaid_receipt(cust_name,cust_id,h_addr,ppr,month,bill_amt,bank_charges,total_amt,bank,banker,tx_date) VALUES(:cust_name,:cust_id,:h_addr,:ppr,:month,:bill_amt,:bank_charges,:total_amt,:bank,:banker,:tx_date)");
	      $stmt->execute(array(
	         ':cust_name' => $cust_name,
						 ':cust_id' => $cust_id,
						 ':h_addr' => $h_addr, 			
				...
				//delete the invoice
				$stmt = $db->query("DELETE FROM postpaid_invoice WHERE ppr = '$ppr' AND tx_status = 'pending'");
	     //go to print receipt page
       redirect("postpaid_receipt.php?ppr=$ppr");
     }
   }
...
<div class="container my-background">
	<div id="content">
		<div id='login_div5'>
				<h2>Postpaid Bill Processing</h2>
		   <?php 
	...
	<form  role="form" id="prepaid-form" method="post" action="process_postpaid.php"style="width:400px;"> 
		<table width="1200" height="auto" border="0" > 
			...
			<tr>
					<td><span id="sprytextfield2">
						<label for="amt">Bill Amount <br />
							<input type="text" class="StyleTxtField2" id="amt" value="<?php echo $GLOBALS['bill_amt'];?>" disabled>
						</label></span>
					</td>
			</tr>
			<tr>
			...
			<tr>
					<td><span id="sprytextfield2">
						<label for="month">Month <br />
							<input type="text" class="StyleTxtField2" id="month" value="<?php echo $GLOBALS['month'];?>" disabled>
						</label></span>
					</td>
			</tr>
			<tr>
